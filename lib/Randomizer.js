...
class Randomizer {
    ...
    async randomize(options, seed) {
        ...
        if (options.monsterOptions.swapMonsters || options.monsterOptions.swapUniques) {
            console.log('Swapping Monsters');
            const troopMap = entityEditor.shuffleMonsters(options.monsterOptions);

            if (options.monsterOptions.statScaleUnique) {
