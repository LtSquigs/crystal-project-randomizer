...
class Randomizer {
    ...
    async randomize(options, seed) {
        if(!(
            fs.existsSync(this.exeFile) || fs.existsSync(this.contentPath) ||
            fs.existsSync(this.backupExeFile) || fs.existsSync(this.backupContentPath)
        )) {
        ...
        if (options.monsterOptions.swapMonsters || options.monsterOptions.swapBosses) {
            console.log('Swapping Monsters');
            const bossTroops = dbReader.getBossTroops();
            const troopMap = entityEditor.shuffleMonsters(options.monsterOptions, bossTroops);

            if (options.monsterOptions.statScaleBosses || options.monsterOptions.stateScaleNormal) {
                dbReader.scaleTroopStats(troopMap);
            }
        }
