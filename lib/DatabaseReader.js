...
class DatabaseReader {
    ...
    getBossTroops() {
        ...
        delete bossTroops['213']; // TRex is Dumb
    ...
    scaleTroopStats(troopMap) {
        ...
        for(let troopSet of troopMap) {
            ...
            for(let newTroop of newTroopSet) {
                // From the new monsters we want to grab the highest level monster in any of the troops and scale the whole instance by that
                if (newTroop) {
                    // Find the new troop in the database
                    for(let troop of this.databases.troop.json) {
            ...
            if (maxOldLevel !== -1 && maxNewLevel !== -1) {
                for(let id of monsterIds) {
                    if (smallestMonsterScale[id] === undefined) {
                        smallestMonsterScale[id] = 999999;
                    }
                    if (maxOldLevel/maxNewLevel < smallestMonsterScale[id]) {
                        smallestMonsterScale[id] = maxOldLevel/maxNewLevel;
                    }
                }
        ...
        const seenMonsters = {}; // dont scale monsters twice
        for(let troopInfo of troopsToScale) {
            for(let scaleTroop of troopInfo.troops) {
                for(let troop of this.databases.troop.json) {
                    if (troop.ID === scaleTroop.TroopID) {
                        for(let troopMonster of troop.Members) {
                            if(seenMonsters[troopMonster.MonsterID]) continue;
                            for(let monster of this.databases.monster.json) {
                                if (monster.ID === troopMonster.MonsterID && troopInfo.scale == smallestMonsterScale[monster.ID]) {
                                    ...
                                    monster.PAccRating = Math.round(monster.PAccRating * troopInfo.scale);
                                    monster.PEvaRating = Math.round(monster.PEvaRating * troopInfo.scale);
                                    break;
                                }
                            }
                        }
