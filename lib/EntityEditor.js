...
class EntityEditor {
    ...
    shuffleMonsters(options, bossTroops) {
        ...
        const hasBossTroop = (troops) => {
            for(let troop of troops) {
                if(bossTroops.indexOf(troop.TroopID) !== -1) {
                    return true;
                }
            }
            return false;
        ...
        for(let key in this.entityFiles) {
            ...
            for(let entity of entityFile) {
                if(entity.SparkData) {
                    if (!options.swapBosses &&  hasBossTroop(entity.SparkData.TroopPages)) {
                        continue;
                    }
                    if (!options.swapMonsters &&  !hasBossTroop(entity.SparkData.TroopPages)) {
                        continue;
                    }
            ...
            for(let entity of entityFile) {
                if(entity.SparkData) {
                    ...
                    if(options.bossesToBosses) { // complex shuffle
                        ...
                        // the same number of boss and non boss monsters in the new entity list as the
                        // old one
                        while(hasBossTroop(newTroops) !== hasBossTroop(entity.SparkData.TroopPages)) {
                            shuffledTroops.push(newTroops);
                            newTroops = shuffledTroops.shift();
                        }
                        if (options.statScaleBosses && hasBossTroop(newTroops)) {
                            troopMap.push([entity.SparkData.TroopPages, newTroops])
                        }
                        if (options.statScaleNormal && !hasBossTroop(newTroops)) {
                            troopMap.push([entity.SparkData.TroopPages, newTroops])
                        }
